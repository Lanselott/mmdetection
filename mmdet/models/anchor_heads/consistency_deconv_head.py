...
class ConsistencyDeconvHead(nn.Module):
    """
    ...
    def __init__(self,
        ...
        super(ConsistencyDeconvHead, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.reg_deconvs = nn.ModuleList()
        self.cls_deconvs = nn.ModuleList()
        ...
        for i in range(self.stacked_convs):
            ...
            self.reg_convs.append(
                ConvModule(
                    ...
                    norm_cfg=self.norm_cfg,
                    bias=self.norm_cfg is None))
        # Deconv on reg and cls
        # Upsampled small feature size may different 
        for i in range(len(self.strides)):
            ratio = 2 ** i
            self.reg_deconvs.append(nn.ConvTranspose2d(
                    self.feat_channels,
                    self.feat_channels,
                    ratio,
                    stride=ratio))
            self.cls_deconvs.append(nn.ConvTranspose2d(
                    self.feat_channels,
                    self.feat_channels,
                    ratio,
                    stride=ratio))
    ...
    def init_weights(self):
        ...
        for layer in [self.cls_deconvs, self.reg_deconvs]:
            for m in layer:
    ...
    def forward_single(self, x, upsample_size, scale, level):
        ...
        cls_feat = self.cls_deconvs[level](cls_feat)        
        ...
        reg_feat = self.reg_deconvs[level](reg_feat)
