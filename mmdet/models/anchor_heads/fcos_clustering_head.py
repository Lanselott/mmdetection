...
class FCOSClusteringHead(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 loss_clustering=dict(type='MSELoss', loss_weight=1.0),
        ...
        super(FCOSClusteringHead, self).__init__()
        ...
        self.loss_clustering = build_loss(loss_clustering)
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            pos_cluster_preds = (pos_points + pos_offset_preds) * center_factor
            pos_cluster_preds[:, 0] /= max_points_x
            pos_cluster_preds[:, 1] /= max_points_y
            pos_decoded_bbox_preds = distance2bbox(pos_points, pos_bbox_preds)
            pos_decoded_target_preds = distance2bbox(pos_points,
                                                     pos_bbox_targets)
            center_bbox_x = (pos_decoded_target_preds[:, 2] -
                             pos_decoded_target_preds[:, 0]).reshape(-1, 1)
            center_bbox_y = (pos_decoded_target_preds[:, 3] -
                             pos_decoded_target_preds[:, 1]).reshape(-1, 1)
            pos_bbox_center_targets = torch.cat([center_bbox_x, center_bbox_y],
                                                1) * center_factor
            pos_bbox_center_targets[:, 0] /= max_points_x
            pos_bbox_center_targets[:, 1] /= max_points_y
            ...
            loss_clustering = self.loss_clustering(pos_cluster_preds,
                                                   pos_bbox_center_targets)
        else:
            loss_bbox = pos_bbox_preds.sum()
            loss_clustering = pos_bbox_preds.sum()
