...
class DDBV3CSHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(DDBV3CSHead, self).__init__()
    ...
    def fcos_target_single(self, stride, gt_bboxes, gt_labels, points, regress_ranges):
        ...
        stride = stride * self.sample_ratio
        ...
        center_gt[..., 0] = torch.where(xmin > gt_bboxes[..., 0], xmin, gt_bboxes[..., 0])
        center_gt[..., 1] = torch.where(ymin > gt_bboxes[..., 1], ymin, gt_bboxes[..., 1])
        center_gt[..., 2] = torch.where(xmax > gt_bboxes[..., 2], gt_bboxes[..., 2], xmax)
        center_gt[..., 3] = torch.where(ymax > gt_bboxes[..., 3], gt_bboxes[..., 3], ymax)
        pos_left = xs - center_gt[..., 0]
        pos_right = center_gt[..., 2] - xs
        pos_top = ys - center_gt[..., 1]
        pos_bottom = center_gt[..., 3] - ys

        center_bbox = torch.stack((pos_left, pos_top, pos_right, pos_bottom), -1)
        bbox_targets = torch.stack((left, top, right, bottom), -1)
        # bbox_center_targets = torch.stack((center_left, center_top, center_right, center_bottom), -1)

        # condition1: inside a gt bbox
        # inside_gt_bbox_mask = bbox_targets.min(-1)[0] > 0
        inside_gt_bbox_center_mask = center_bbox.min(-1)[0] > 0
