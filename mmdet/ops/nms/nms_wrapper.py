...
def nms_v2(dets, iou_thr, c_thr, device_id=None):
    ...
    dets = dets[:, :5]
    dets_x1 = dets[inds[:, 0], 0].reshape(-1, 1)
    dets_y1 = dets[inds[:, 1], 1].reshape(-1, 1)
    dets_x2 = dets[inds[:, 2], 2].reshape(-1, 1)
    dets_y2 = dets[inds[:, 3], 3].reshape(-1, 1)
    dets_score = dets[inds[:, 4], 4].reshape(-1, 1)
